...
import { Heart, FileText, Shield, Phone, Accessibility, Trash2 } from 'lucide-react';
...
export function SiteFooter() {
  ...
  const hasMoreRegions = REGIONS.length > REGIONS_SHOWN;
  return (
    <>
      {/* 11.3: the owner-register link used to sit inside the nav column as just another list
          item — easy to miss and visually cramped next to "favorites"/"contact". A full-width
          strip above the footer gives it the weight an actual conversion CTA needs. */}
      <div className="site-footer-cta">
        <div className="site-footer-cta__inner container">
          <span className="site-footer-cta__text">{t.footerCtaText}</span>
          <Link to="/register" className="site-footer-cta__btn">{t.footerOwnerRegister}</Link>
        </div>
      </div>
        ...
        <div className="site-footer__inner container">
          ...
          <div className="site-footer__cols">
            <div className="site-footer__col">
              ...
              {hasMoreRegions && (
                <Link to="/" className="site-footer__nav-link site-footer__nav-link--muted">{t.footerAllRegions}</Link>
              )}
            </div>
            <div className="site-footer__col">
