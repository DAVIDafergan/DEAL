...
/**
 * Extractor entry point (Step 3.3, revised for keyless operation). RuleBasedExtractor
 * (ruleBasedExtractor.js) is the real default — deterministic, works entirely off the meta
 * tags/JSON-LD/regex matches pageFetcher.js already parses out of the HTML, no external API, no
 * cost, no key required. Claude is only used as an *optional* enhancement when ANTHROPIC_API_KEY
 * is explicitly configured — this codebase must extract real properties with zero keys and zero
 * external APIs by default. Validates against the zod schema, retries once, then gives up and
 * reports a rejection (never invents data to force a pass). Enforces the description-rewrite
 * rule independently of schema validity.
 */
export async function extractProperty({ pageText, sourceUrl, hints = {}, metaTags = {}, jsonLd = [], phoneMatches = [], whatsappLinks = [] }) {
  const apiKey = process.env.ANTHROPIC_API_KEY || null;
  const attempt = () => (apiKey
    ? callClaude(pageText, sourceUrl, apiKey)
    : ruleBasedExtract({ text: pageText, metaTags, jsonLd, phoneMatches, whatsappLinks, sourceUrl, hints }));
  ...
  if (!validated?.success) {
    return {
      ...
      errors: validated?.error?.issues?.map((i) => `${i.path.join('.')}: ${i.message}`) || (callError ? [callError.message] : undefined),
      usedRuleBasedExtractor: !apiKey,
    };
  }
  ...
  return { ok: true, data, usedRuleBasedExtractor: !apiKey, descriptionRejectedForCopying };
}
